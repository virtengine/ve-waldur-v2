...
from ..models import OpenNebulaNetwork, OpenNebulaTenant, OpenNebulaVolume
...
class OpenNebulaBackendVolumeTest(unittest.TestCase):
    ...
    def test_pull_volumes(self):
        self.backend.client.list_volumes.return_value = [
            mock.Mock(ID=1, NAME="vol1", DESCRIPTION="desc", SIZE=10)
        ]
    ...
    def test_create_volume(self):
        ...
        self.backend.client.get_volume.return_value = mock.Mock(
            ID=1, NAME="vol1", DESCRIPTION="desc", SIZE=10
        )
    ...
    def test_resize_and_save_as_image(self):
        ...
        self.backend.client.save_disk_as_image.assert_called_once_with(
            "vm1", "disk1", "img", "", -1
        )
...
class OpenNebulaBackendNetworkTest(unittest.TestCase):
    ...
    def test_reserve_ar(self):
        ...
        self.backend.client.reserve_ar.assert_called_once_with(
            "net1", "reservation_template"
        )
...
class OpenNebulaBackendTenantTest(unittest.TestCase):
    ...
    def test_pull_tenants(self):
        self.backend.client.list_groups.return_value = [
            mock.Mock(ID=1, NAME="tenant1", DESCRIPTION="desc")
        ]
    ...
    def test_create_tenant(self):
        tenant = mock.Mock(spec=OpenNebulaTenant)
        tenant.name = "tenant1"
        tenant.description = "desc"
        tenant.service_settings = self.settings
        tenant.save = mock.Mock()
        tenant.set_ok = mock.Mock()

        self.backend.client.create_group.return_value = 1

        result = self.backend.create_tenant(tenant)

        self.assertEqual(result, tenant)
        self.backend.client.create_group.assert_called_once_with("tenant1", "desc")
        tenant.save.assert_has_calls(
            [
                mock.call(update_fields=["backend_id", "service_settings"]),
                mock.call(update_fields=["state"]),
            ]
        )
        tenant.set_ok.assert_called_once()
...
class OpenNebulaBackendVMTest(unittest.TestCase):
    ...
    def test_pull_vms(self):
        self.backend.client.list_vms.return_value = [
            mock.Mock(ID=1, NAME="vm1", TEMPLATE=mock.Mock(CPU=2, MEMORY=2048))
        ]
    ...
    def test_create_vm(self):
        ...
        self.backend.client.get_vm.return_value = mock.Mock(
            ID=1, NAME="vm1", TEMPLATE=mock.Mock(CPU=2, MEMORY=2048)
        )
        vm = self.backend.create_vm(
            self.tenant, "vm1", template_id=1, cpu=2, ram=2048, disk=20
        )
