    ...
    def get_task_signature(cls, host, serialized_host, **kwargs):
        if host.backend_id:
            ...
            return tasks.StateTransitionTask().si(
                serialized_host, state_transition='begin_deleting'
            )
    ...
    def get_task_signature(cls, user, serialized_user, **kwargs):
        ...
        if user.phone:
            ...
            creation_tasks.append(
                SMSTask().si(serialized_settings, message, user.phone)
            )
...
class ServiceSettingsPasswordResetExecutor(executors.ActionExecutor):
    ...
    def get_task_signature(
        cls, service_settings, serialized_service_settings, **kwargs
    ):
        ...
        _tasks = [
            tasks.StateTransitionTask().si(
                serialized_service_settings, state_transition='begin_updating'
            ),
            tasks.BackendMethodTask().si(
                serialized_user, 'update_user', state_transition='begin_updating'
            ),
            UpdateSettingsCredentials().si(
                serialized_service_settings, serialized_user
            ),
        ...
        if user.phone:
            message = 'Zabbix "%s" password: %s' % (user.settings.name, user.password)
            _tasks.append(
                SMSTask().si(serialized_service_settings, message, user.phone)
            )
        return chain(*_tasks)
